...
import { GzhInfo, ArticleInfo, ArticleMeta, PdfInfo, DownloadOption, FilterRuleInfo, Service, NodeWorkerResponse, NwrEnum, DlEventEnum, ProccessRecord } from './service';
 ...
 * @return {*}
 */
async function axiosDlOne(articleInfo: ArticleInfo, reCall = false, failCount = 0) {
  // 如果标题不为空，则是从批量下载过来的，可以提前判断跳过
  if (articleInfo.title && !reCall) {
    const timeStr = articleInfo.datetime ? DateUtil.format(articleInfo.datetime, 'yyyy-MM-dd hh::mm::ss') + '-' : '';
  ...
  // 2024-7-17更新：微信对文章获取接口作了限制，需要有header参数才能使用
  const gzhInfo = articleInfo.gzhInfo;
  
  const trimedArticleUrl = articleInfo.contentUrl
  await axios
    .get(trimedArticleUrl, {
      params: {
  ...
  await dlOne(articleInfo, true, failCount);
}

/**2024-7-18更新
 * @description: 异常下载出路函数。重试，不成功则对生成的文件夹进行删除，并退出下载
 * @param {string} savePath 文章的保存路径
 * @param {ArticleInfo} articleInfo 文章信息
 * @param {number} failCount 已失败次数
 * @return {*}
 */
async function retryOrRecovery(
  savePath:string, 
  articleInfo: ArticleInfo, 
  failCount:number) {
  // 递归删除文件夹
  if (fs.existsSync(savePath)) {
    resp(NwrEnum.FAIL, "删除文件夹：" + savePath)
    FileUtil.rmDir(savePath)
  }
  ...
  await axiosDlOne(articleInfo, true, failCount + 1);
  return;
}
 ...
 * 下载单个页面
 */
async function dlOne(articleInfo: ArticleInfo, saveToDb = true, failCount = 0) {
  // 预处理微信公号文章html
  if (!articleInfo.html) return;
  ...
  logger.info("文章标题", article)

  // 2024-7-10更新
  // 对正文中的标题进行检测，如果标题为空，则判断为文章异常，可能为被封禁或环境异常
  // 触发了公众号的反爬机制，进行重试

  // 2024-7-17更新
  // 对文章内容进行检测，如果出现环境异常页面则结束下载

  // 2024-7-18更新
  // 将异常时恢复现场，删除文件夹，退出下载的过程整合成一个函数，方便调用

  if (article.title == '微信公众平台' || article.textContent.indexOf("当前环境异常，完成验证后即可继续访问。") >= 0) {
    retryOrRecovery(savePath, articleInfo, failCount + 1)
    return;
  }
    ...
    resp(NwrEnum.FAIL, `【${articleInfo.title}】文章标题为空!`);
    return;
  }
...
async function batchDownloadFromWeb() {
  ...
  // 2024-7-12更新
  // 读取上次保存的数据列表的offset值，用于接续下载
  let next_offset = 0
  const proccessRecordPath = path.join(downloadOption.savePath || "", "ProccessRecord.cnf")
  if(fs.existsSync(proccessRecordPath)) {
    const lastRecord: ProccessRecord = JSON.parse(fs.readFileSync(proccessRecordPath, 'utf8'))
    next_offset = lastRecord.lastOffset
  }
  await downList(next_offset, articleArr, startDate, endDate, articleCount);
...
async function downList(nextOffset: number, articleArr: ArticleInfo[], startDate: Date, endDate: Date, articleCount: number[]) {
  let dataObj;
  logger.info('下载文章列表', `${LIST_URL}&__biz=${GZH_INFO.biz}&key=${GZH_INFO.key}&uin=${GZH_INFO.uin}&pass_ticket=${GZH_INFO.passTicket}&offset=${nextOffset}`);
  ...
  // 2024-7-12更新
  // 问题：微信的文章列表查询可能存在上限，一定次数后会请求失败
  // 解决：在每次获取文章列表前保存此次的参数，避免每次从头开始请求文章列表
  const proccessRecord = new ProccessRecord(nextOffset)
  const proccessRecordPath = path.join(downloadOption.savePath || "", "ProccessRecord.cnf")
  fs.writeFileSync(proccessRecordPath, JSON.stringify(proccessRecord), "utf8")
