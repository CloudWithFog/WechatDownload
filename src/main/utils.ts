...
class StrUtil {
  // 2024-8-6更新：添加修剪url的工具
  public static trimAticleUrl(url: string, remainKeywords: string[]): string{
    ...
    for(let param of params) {
      ...
      if(remainKeywords.indexOf(paramKey) > -1)
        finalUrl += `${param};` 
    }
    return finalUrl;
  }
...
class DateUtil {
  public static format(datetime: Date | string, formatting: string): string {
    ...
    formatting = this.parse(formatting, /[h|H]+/, hours, '00');
